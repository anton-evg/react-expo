...
function Header() {
  ...
  return <header className="react-home__header"><div className="react-home__header-content react-home__container"><a className="react-home__logo" href="/" aria-label="ЭКСПО НЕО — главная страница"><img src="/assets/logo.png" alt="Логотип ЭКСПО НЕО" /></a><nav className={`react-home__navigation ${isOpen ? "react-home__navigation--open" : ""}`} aria-label="Основная навигация">{navigationItems.map(([label, href]) => <a href={href} key={href}>{label}</a>)}</nav><a className="react-home__phone" href="tel:+74951426880">+7 (495) 142 68 80</a><button className={`react-home__menu-button ${isOpen ? "react-home__menu-button--open" : ""}`} type="button" aria-label={isOpen ? "Закрыть меню" : "Открыть меню"} aria-expanded={isOpen} onClick={() => setIsOpen(!isOpen)}><span></span><span></span></button></div></header>;
}
